from .figures.parabola import Parabola
...
class Visualizer:
    ...
    def add_parabola(self, data, **kwargs):
        parabola = Parabola(data, kwargs)
        self.data.append(parabola)
        return parabola
    ...
    def show(self, x_lim=None, y_lim=None):
        Plot.show(self.plot_data, self.data, x_lim, y_lim)

    def save(self, filename='plot'):
        Plot.save(self.plot_data, self.data, filename)

    def show_gif(self, interval=256, xlim=None, ylim=None):
        gif = Plot.show_gif(self.plot_data, self.data, interval, xlim, ylim)
        return gif

    def save_gif(self, filename='animation', interval=256, xlim=None, ylim=None):
        Plot.save_gif(self.plot_data, self.data,
                      interval, filename, xlim, ylim)
